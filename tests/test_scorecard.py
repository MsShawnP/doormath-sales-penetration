...
class TestScorecardData:
    ...
    def test_retailer_rows_have_required_keys(self):
        ...
        data = _get_scorecard_data()
        expected = {"name", "carrying", "addressable", "penetration", "acv_pct", "tdp", "delta"}
        for row in data["retailer_rows"]:
            assert set(row.keys()) == expected, (
                f"Row for {row.get('name', '?')} missing keys: {expected - set(row.keys())}"
            )
    ...
    def test_product_line_rows_have_required_keys(self):
        ...
        data = _get_scorecard_data()
        expected = {"name", "carrying", "addressable", "penetration", "acv_pct", "delta"}
