...
from cinderhaven_store_universe import (
    get_stores, get_auth_matrix, get_scan_data, get_slow_leak_config,
)
from cinderhaven_store_universe.constants import RETAILERS

from app.calculations import (
    quarters_in_range, calc_acv_pct, calc_tdp, calc_penetration_rate,
    calc_period_delta,
)
...
for i, ret_id in enumerate(_RETAILER_IDS_SORTED):
    idx = _RETAILER_COLOR_INDICES[i] if i < len(_RETAILER_COLOR_INDICES) else i % len(TEAL_SEQUENTIAL)
...
def _compute_slow_leak_annotations(filters, quarters):
    ...
    for sku_id, config in _slow_leak.items():
        ...
        if drop_pp > 0.20:
            ...
            auth_for_sku = _auth[
                (_auth['authorized']) & (_auth['sku_id'] == sku_id)
            ]
            if retailers:
                auth_for_sku = auth_for_sku[
                    auth_for_sku['retailer_id'].isin(retailers)
                ]
