...
def _compute_scorecard_data(filters):
    ...
    for ret_id in active_retailers:
        ...
        auth_filtered = _auth[
            _auth['authorized'] & _auth['retailer_id'].isin(ret_filter)
        ]
        if product_lines:
    ...
    for pl_prefix in active_pls:
        ...
        if retailers:
            auth_filtered = auth_filtered[
                auth_filtered['retailer_id'].isin(retailers)
            ]
        auth_filtered = auth_filtered[
            auth_filtered['sku_id'].str.split('-').str[1].isin(pl_filter)
        ]
        if sku:
            auth_filtered = auth_filtered[auth_filtered['sku_id'] == sku]
        addressable = auth_filtered['store_id'].nunique()
...
def _update_scorecard(filter_json):
    ...
    return (
        hero_text, trend_text, trend_style,
        retailer_table, product_line_table, exceptions_list,
        annotation,
    )
