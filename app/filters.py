...
RETAILER_OPTIONS = [
    {'label': info['name'], 'value': ret_id}
    for ret_id, info in RETAILERS.items()
]
ALL_RETAILER_IDS = [opt['value'] for opt in RETAILER_OPTIONS]

PRODUCT_LINE_OPTIONS = [
    {'label': info['name'], 'value': prefix}
...
def register_filter_callbacks():
    ...
    def _update_sku_visibility(product_lines, current_sku):
        ...
        if product_lines and len(product_lines) == 1:
            ...
            return (
                {'minWidth': '180px', 'flex': '1', 'display': 'flex',
                 'flexDirection': 'column', 'gap': 'var(--ll-space-xs)'},
                options,
                kept_sku,
            )
        # Hide and reset
